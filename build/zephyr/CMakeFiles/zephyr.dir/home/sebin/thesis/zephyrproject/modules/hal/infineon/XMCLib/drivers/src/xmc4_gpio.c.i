# 1 "/home/sebin/thesis/zephyrproject/modules/hal/infineon/XMCLib/drivers/src/xmc4_gpio.c"
...
# 1 "<command-line>" 2
# 1 "/home/sebin/thesis/zephyrproject/modules/hal/infineon/XMCLib/drivers/src/xmc4_gpio.c"
# 57 "/home/sebin/thesis/zephyrproject/modules/hal/infineon/XMCLib/drivers/src/xmc4_gpio.c"
...
# 58 "/home/sebin/thesis/zephyrproject/modules/hal/infineon/XMCLib/drivers/src/xmc4_gpio.c" 2
# 74 "/home/sebin/thesis/zephyrproject/modules/hal/infineon/XMCLib/drivers/src/xmc4_gpio.c"
void XMC_GPIO_Init(XMC_GPIO_PORT_t *const port, const uint8_t pin, const XMC_GPIO_CONFIG_t *const config)
{
  ;
  ;
  ...
  port->IOCR[pin >> 2U] &= (uint32_t)~((0xf8UL) << ((8U) * (pin & 0x3U)));


  port->HWSEL &= ~(uint32_t)((uint32_t)(0x3UL) << ((uint32_t)pin << 1U));



  if (((port == ((XMC_GPIO_PORT_t *) 0x48028E00UL)) || (port == ((XMC_GPIO_PORT_t *) 0x48028F00UL))))
  {
    port->PDISC &= ~(uint32_t)((uint32_t)0x1U << pin);
  }
  else
  {
    if ((config->mode & XMC_GPIO_MODE_OUTPUT_PUSH_PULL) != 0)
    {
      ...
      port->PDR[pin >> 3U] &= (uint32_t)~((0x7UL) << ((uint32_t)(4U) * ((uint32_t)pin & 0x7U)));
      port->PDR[pin >> 3U] |= (uint32_t)config->output_strength << ((uint32_t)(4U) * ((uint32_t)pin & 0x7U));
    }
  }


  port->IOCR[pin >> 2U] |= (uint32_t)config->mode << ((uint32_t)(8U) * ((uint32_t)pin & 0x3U));
}
...
void XMC_GPIO_SetOutputStrength(XMC_GPIO_PORT_t *const port, const uint8_t pin, XMC_GPIO_OUTPUT_STRENGTH_t strength)
{
  ...
  port->PDR[pin >> 3U] &= (uint32_t)~((uint32_t)(0x7UL) << ((uint32_t)(4U) * ((uint32_t)pin & 0x7U)));
  port->PDR[pin >> 3U] |= (uint32_t)strength << ((uint32_t)(4U) * ((uint32_t)pin & 0x7U));
}
